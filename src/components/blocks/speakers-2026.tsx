...
export function Speakers2026() {
  const t = useTranslations('speakers2026')
  ...
  useEffect(() => {
    async function fetchSpeakers() {
      try {
        const response = await fetch('/api/speakers?year=2026')
        if (!response.ok) {
  ...
  return (
    <section id="predavaci" ref={ref} className="relative py-20 sm:py-32 overflow-hidden">
      {/* Background effects */}
