...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, transparent = true }: {
  ...
  // Prevent scrolling when touching the lanyard canvas on mobile
  useEffect(() => {
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: { maxSpeed?: number; minSpeed?: number; isMobile?: boolean }) {
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        {/* @ts-expect-error - meshLineGeometry is extended via react-three/fiber */}
        <meshLineGeometry />
        {/* @ts-expect-error - meshLineMaterial is extended via react-three/fiber */}
        <meshLineMaterial
          color="white"
          depthTest={false}
